...
- Два бизнес-кейса (игра ABC и sales-кейс) с поддержкой "умеренного" типа поведения ML-агентов.
...
3) Дальнейшее развитие решения: проанализировать сценарий переговоров, декомпоизировать его и разбить перегворы на этапы. Таким образом получится лучше контролировать 
...
* Функционал пилота:
- Работающий Telegram-интерфейс для симуляций.
- Поддержка одного бизнес-кейса (игра ABC).
- Один тип поведения ML-агента — "умеренный".

* Цель пилота:
- Проверить качество симуляции и реалистичность поведения ML-агентов.
- Собрать пользовательский фидбек для дальнейшего улучшения.
- Убедиться в работоспособности интерфейса и модели на практике.

* Участники пилота:
- Группа тестовых пользователей (AB Group), включая экспертов Product Owner и Data Scientist.
- Примерный объем: 5-10 пользователей.

* Способ оценки пилота:
- Сбор данных о взаимодействии пользователей:
- Логирование всех диалогов, включая ответы агентов и действия пользователей.
- Сбор времени отклика ML-агентов для каждой симуляции.
- Сохранение данных о завершенных и незавершенных кейсах.

* Заполнение анкет после завершения симуляций:
- Оценка реалистичности поведения ML-агента (по шкале 1–5).
- Оценка удобства интерфейса (по шкале 1–5).
- Оценка полезности симуляции для улучшения навыков (по шкале 1–5).
- Сбор предложений по улучшению системы.

#### 3.2. Что считаем успешным пилотом
- Реалистичность поведения ML-агента:
- 80% решений должны быть логичными по мнению пользователей.
- 70% пользователей должны оценить симуляцию на 4+ по шкале полезности.
- Средний отклик не должен превышать 15 секунд.
- Более 75% пользователей должны успешно завершить симуляцию.
